...
export default function Kardex() {
  ...
  // MAP optimizado de reactivos
  const reactivoMap = useMemo(() => {
    return Object.fromEntries((reactivos || []).map(r => [r.id, r]));
  }, [reactivos]);
  ...
  const stockMap = useMemo(() => {
    ...
    (movimientos || []).forEach((m) => {
      if (!map[m.reactivo_id]) map[m.reactivo_id] = [];
      map[m.reactivo_id].push(m);
    });
    return map;
  }, [movimientos]);

  // FILTRO LIMPIO (sin responsable)
  const filtered = useMemo(() => {
    return (movimientos || []).filter((m) => {
      if (selectedReactivo !== 'all' && m.reactivo_id !== selectedReactivo) return false;

      if (search) {
        const r = reactivoMap[m.reactivo_id];
  ...
  }, [movimientos, selectedReactivo, search, reactivoMap]);
    ...
    <div className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-4 py-3">Reactivo</th>
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-muted-foreground text-xs whitespace-nowrap">
                        {moment(m.created_at || m.created_date).format('DD/MM/YYYY HH:mm')}
                      </td>
      ...
      <Dialog open={showReactivoPicker} onOpenChange={setShowReactivoPicker}>
        <DialogContent className="max-w-sm">
          ...
          <Select onValueChange={(id) => {
            const r = reactivoMap[id];
            if (r) {
              openMovForm(r);
              setShowReactivoPicker(false);
            }
          }}>
            <SelectTrigger>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              Registrar movimiento — {formReactivo?.nombre}
            </DialogTitle>
          </DialogHeader>
